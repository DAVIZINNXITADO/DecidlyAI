...
function useCreditData() {
  ...
  const load = useCallback(async () => { const { data: auth } = await supabase.auth.getUser(); const user = auth.user; if (!user) return; const [{ data: credits }, { data: history }, { data: referral }] = await Promise.all([supabase.from("ai_credits").select("free_credits,purchased_credits,total_credits,daily_credits_used,daily_credits_limit").eq("user_id", user.id).maybeSingle(), supabase.from("credit_events").select("id,event_type,amount,description,created_at").eq("user_id", user.id).order("created_at", { ascending: false }).limit(100), supabase.from("referral_codes").select("code").eq("user_id", user.id).maybeSingle()]); if (credits) setWallet({ free_credits: Number(credits.free_credits || 0), purchased_credits: Number(credits.purchased_credits || 0), total_credits: Number(credits.total_credits || 0), daily_credits_used: Number(credits.daily_credits_used || 0), daily_credits_limit: Number(credits.daily_credits_limit || 10) }); setEvents((history || []) as Event[]); if (referral?.code) setCode(referral.code); }, []);
  useEffect(() => { void load(); const timer = window.setInterval(() => void load(), 1000); return () => window.clearInterval(timer); }, [load]); return { wallet, events, code };
}
...
export function FreePage() { 
  const { wallet, code } = useCreditData(); 
